...
// import { SuccessNotification } from "../middleware/notifications/index";
import fs from "fs";
...
export const getAlla = (Model, isUser) => async (req, res, next) => {
  try {
    ...
    const total = await Model.find(myQuery).count();
    // console.log("+++++++++>", total);

    // const f = new APIFeatures(Model.find(), req.query).filter();
    // const count = f.query.countDocuments();
...
//  * @static
//  * @req.params {object}
//  * @returns {object} with data Retrieved
//  */
...
export const getAll = (Model, isUser) => async (req, res) => {
  try {
    const { field } = req.query;
    let joined = field?.toString().split(",").join(" ") || "-password -pin";
